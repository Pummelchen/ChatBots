// ChatBotsCoreTests — the reader's idle timeout is its own value
//
...
// work. At the shipped 10-second default it is reachable in ordinary use, because the fix that moved
...
@MainActor
@Suite("A silent stream is not killed by the request timeout", .serialized, TransportSerialized())
