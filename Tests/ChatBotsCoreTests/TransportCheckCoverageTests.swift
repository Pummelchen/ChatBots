// ChatBotsCoreTests — the installer's smoke test, actually run.
//
...
// spawns has to use the identity the check pins. That identity is not reachable from here, so the test
// below covers the branch that fails before any of it, and the passing path is blocked on that gap rather
...
@MainActor
@Suite(
    "The installer's transport smoke test", .serialized, TransportSerialized()
)
