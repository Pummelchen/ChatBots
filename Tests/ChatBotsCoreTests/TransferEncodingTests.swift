// ChatBotsCoreTests — a body the server never decoded must not run its route.
//
...
// field was not sent". Measured against the committed version, a chunked `POST /api/topic` is
...
@Suite("A body the server cannot frame is refused, not read as empty")
...
@MainActor
@Suite("A chunked request does not change the room")
