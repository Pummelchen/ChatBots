// ChatBotsCoreTests — the fingerprint comments match the trust the client gives them
//
...
@Suite("The certificate comments match the trust the client gives them")
struct CertificatePinClaimTests {
    ...
    /// them — the same reason the shell lint stopped deciding its scope by a path list.
    ...
    func storeStatesTheTrade() throws {
        let store = flattened(try text(of: "Sources/ChatBotsCore/Transport/CertificateStore.swift"))
        #expect(
    ...
    func policyAndClientAgree() throws {
        ...
        let client = flattened(try text(of: "Sources/ChatBotsCore/Transport/WebTransportClient.swift"))
    ...
    func fingerprintStillExposed() throws {
        let store = try text(of: "Sources/ChatBotsCore/Transport/CertificateStore.swift")
        let server = try text(of: "Sources/ChatBotsCore/Transport/WebTransportServer.swift")
        let check = try text(of: "Sources/ChatBotsCore/Transport/TransportCheck.swift")
