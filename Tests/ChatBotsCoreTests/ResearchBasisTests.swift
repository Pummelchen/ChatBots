// ChatBotsCoreTests — a basis marker must be a whole word
//
// `hasBasis` tested for markers with a bare `contains`, so "statistic" matched "statistically"
// and "source" matched "resource". That looseness became load-bearing: coverage now depends on
// `hasBasis`, so every claim that a subject has been answered rested on a matcher that could
// qualify a turn on a word that merely contained a marker. The bar that raised was therefore not
// as high as it read.

import ChatBotsCore
import Testing

@Suite("A basis marker is a whole word")
@MainActor
struct ResearchBasisTests {
    ...
    func theCoverageBarMoves() {
        ...
        // does not, so the subject stays open (the coverage rule needs a real basis from both seats).
