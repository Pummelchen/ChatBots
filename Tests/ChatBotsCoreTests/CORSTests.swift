// ChatBotsCoreTests — the HTTP surface grants no cross-origin access
//
...
struct CORSTests {
    ...
    func preflightIsNotAnswered() async throws {
        ...
        // This asserted 404 — "there is no OPTIONS route" — which was the earlier mechanism's
        // byproduct: the request fell through to the default. The origin rule refuses a cross-origin request
        // explicitly instead, so the honest answer is now 403 and it names the reason. The property
        // under test is unchanged: a cross-origin write gains nothing.
