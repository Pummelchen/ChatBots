// ChatBotsCoreTests — BMP and TIFF are converted, not sent as undocumented types (audit A101)
//
// The magic-byte sniffer recognised BMP and TIFF and the intake path sent them as `image/bmp`
// and `image/tiff`. The API documentation A52's lane checked lists png, jpeg, webp and gif, so
// those attachments were likely rejected. This is A52's defect in the other direction — A52
// fixed a type the app offered but did not send; this was a type the app sent but the API may
// not take.
//
// The judgement is A52's: convert what can be converted rather than send an undocumented type
...
private let realPNG = Data(base64Encoded: """
    iVBORw0KGgoAAAANSUhEUgAAAAEAAAABCAYAAAAfFcSJAAAADUlEQVR42mP8z8BQDwAEhQGAhKmMIQAAAABJRU5ErkJggg==
    """)!

@Suite("BMP and TIFF are converted, not sent as undocumented types (audit A101)")
struct AuditClientsImageTests {
    ...
    /// the format A52 and A101 are about.
    ...
    /// A type the API does accept must not be touched, exactly as A52 requires.
