// ChatBotsCoreTests — A96: affinity scores on words, not fragments
//
// `affinity(_:for:)` scored with `haystack.contains($1)` — the same bare-substring shape A67
...
// it was recorded as S3. The fix is the word-boundary rule coverage already uses.

import ChatBotsCore
import Testing

@Suite("Affinity matches words, not fragments (A96)")
@MainActor
struct AuditResearchAffinityTests {
