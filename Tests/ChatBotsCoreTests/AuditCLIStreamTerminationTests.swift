// ChatBotsCoreTests — a reported failure ends the read, it does not wait out the timeout (audit A107)
//
...
// had already given. The same shape applied to a top-level `error` event. A54's terminal-event
...
@MainActor
@Suite("A reported failure ends the read (audit A107)")
struct AuditCLIStreamTerminationTests {
