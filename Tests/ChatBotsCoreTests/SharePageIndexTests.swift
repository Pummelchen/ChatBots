// ChatBotsCoreTests — a share link does not stall the engine
//
// A kept conversation lives in one index file, `conversations.json`, holding every record — that is
// deliberate, and that is why the write is a single atomic replacement of that one file. A *read* of
...
//
// The fix is not a new storage format (that is a migration, not a scoped correction) but the actor the read runs on:
// `ConversationStore.listOffMainActor()` and `conversationOffMainActor(id:)` hand the decode to a
// detached task, and the request paths that trigger it use them. This is the shape used for
...
/// what the test below must not do, so the wait happens on a dispatch queue. The same helper the
...
@Suite("A share link and the conversation index")
