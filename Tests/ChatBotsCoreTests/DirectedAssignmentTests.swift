// ChatBotsCoreTests — the directed assignment is a marker, not a phrase
//
// The coverage rule made a subject covered from a single seat when that seat answers the moderator's own
...
@Suite("A directed assignment is carried structurally")
@MainActor
struct DirectedAssignmentTests {
    ...
    func rewordedAssignmentStillCounts() {
        ...
        // The copy edit this is about: the instruction no longer contains the old prefix.
...
@Suite("The assignment marker survives storage")
@MainActor
struct DirectedAssignmentStoreTests {
