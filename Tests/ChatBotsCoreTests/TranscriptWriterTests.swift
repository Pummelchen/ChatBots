...
struct TranscriptWriterTests {
    ...
    private func conversation() throws -> (topic: String, turns: [Turn], seats: [AgentSpec]) {
        ...
        let turns = [
            Turn(
                ...
                content: "Why are eggs not round?", timestamp: try date("2026-12-25 13:15:04")),
            Turn(
                sequence: 2, speakerName: "System", kind: .introduction,
                content: "This is an open discussion…", timestamp: try date("2026-12-25 13:15:04")),
            Turn(
                sequence: 3, speakerID: "Agent 1", speakerName: "Mira", kind: .chat,
                content: "Because of how shells form.", timestamp: try date("2026-12-25 13:15:41")),
            Turn(
                sequence: 4, speakerID: "Agent 2", speakerName: "Otto", kind: .chat,
                content: "Which part of that is actually established?",
                timestamp: try date("2026-12-25 13:16:02")),
            Turn(
                sequence: 5, speakerName: "Moderator", kind: .steering,
                content: "Stay on the shell question.", timestamp: try date("2026-12-25 13:16:30")),
            Turn(
                sequence: 6, speakerID: "Agent 1", speakerName: "Mira", kind: .chat,
                content: "Shell thickness scales with curvature.",
                timestamp: try date("2026-12-25 13:17:10")),
        ]
    ...
    func timestampFormat() throws {
        #expect(TranscriptWriter.timestamp(try date("2026-12-25 13:15:04")) == "2026-12-25 13:15:04")
    ...
    func timestampIsLocaleIndependent() throws {
        ...
        // read individually.
        let instant = try date("2026-12-25 13:15:04")
    ...
    func orderIsChronological() throws {
        let data = try conversation()
        let text = TranscriptWriter.text(
            topic: data.topic, turns: data.turns, participants: data.seats,
            exportedAt: try date("2026-12-25 13:20:00"))
    ...
    func messagesAreStampedAndAttributed() throws {
        let data = try conversation()
        let text = TranscriptWriter.text(
            topic: data.topic, turns: data.turns, participants: data.seats,
            exportedAt: try date("2026-12-25 13:20:00"))
    ...
    func headerIsInformative() throws {
        let data = try conversation()
        let text = TranscriptWriter.text(
            topic: data.topic, turns: data.turns, participants: data.seats,
            exportedAt: try date("2026-12-25 13:20:00"))
    ...
    func summaryIsIncluded() throws {
        var data = try conversation()
        data.turns.append(
            Turn(
                sequence: 7, speakerName: "Condensed", kind: .summary,
                content: "They agreed the mechanism is structural.",
                timestamp: try date("2026-12-25 13:18:00")))
        let text = TranscriptWriter.text(
            topic: data.topic, turns: data.turns, participants: data.seats,
            exportedAt: try date("2026-12-25 13:20:00"))
    ...
    func multiLineMessageIsIndented() throws {
        var data = try conversation()
        data.turns.append(
            Turn(
    ...
    func toolTrafficIsIncluded() throws {
        var data = try conversation()
        data.turns.append(
            Turn(
                sequence: 7, speakerID: "Agent 2", speakerName: "Otto", kind: .tool,
                content: "web_search: 5 results", timestamp: try date("2026-12-25 13:18:00")))
        let text = TranscriptWriter.text(
            topic: data.topic, turns: data.turns, participants: data.seats,
            exportedAt: try date("2026-12-25 13:20:00"))
    ...
    func suggestedFilename() throws {
        let name = TranscriptWriter.suggestedFilename(
            topic: "Why are eggs not round? / really!", at: try date("2026-12-25 13:15:04"))
    ...
    func longTopicFilename() throws {
        let name = TranscriptWriter.suggestedFilename(
            topic: String(repeating: "egg ", count: 200), at: try date("2026-12-25 13:15:04"))
        #expect(name.count < 120)
        #expect(name.hasSuffix(".txt"))
    }

    @Test("An empty topic still yields a filename")
    func emptyTopicFilename() throws {
        let name = TranscriptWriter.suggestedFilename(topic: "   ", at: try date("2026-12-25 13:15:04"))
        #expect(name.hasPrefix("ChatBots "))
        #expect(name.hasSuffix(".txt"))
    }

    @Test("The export is valid UTF-8 that round-trips")
    func exportIsValidUTF8() throws {
        var data = try conversation()
        data.turns.append(
            Turn(
                sequence: 7, speakerID: "Agent 1", speakerName: "Mira", kind: .chat,
    ...
    func newlinesCannotForgeEntries() throws {
        ...
        let text = TranscriptWriter.text(
            ...
            participants: [], exportedAt: try date("2026-12-25 13:21:00"))
