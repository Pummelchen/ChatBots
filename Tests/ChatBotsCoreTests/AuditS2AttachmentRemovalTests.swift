// ChatBotsCoreTests — removing an attachment while the room is running (A173).
//
...
@MainActor
@Suite("An attachment can be removed while the conversation runs (A173)")
struct AttachmentRemovalTests {
    ...
    func removalMidRunIsRefused() async {
        ...
        // with the app's ✕ always enabled so a click did nothing at all (A173).
