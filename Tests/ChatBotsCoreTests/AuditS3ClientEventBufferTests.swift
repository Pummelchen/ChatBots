// ChatBotsCoreTests — the client's event buffer, and a frame it cannot read (A158).
//
// The server bounded each client's event stream at 256 (`.bufferingNewest(256)`, A41/A92) and the client
...
@MainActor
@Suite("The client's event stream is bounded where the server's is (A158)", .serialized, TransportSerialized())
