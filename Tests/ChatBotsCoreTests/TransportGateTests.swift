// ChatBotsCoreTests — the transport gate survives cancellation.
//
// `TransportGate` serialises the five suites that drive a real QUIC listener, because two of them
// overlapping aborts the whole test process inside Network.framework. It parked a waiter on a
...
@Suite("The transport gate survives cancellation")
struct TransportGateTests {
