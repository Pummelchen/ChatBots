// ChatBotsCoreTests — coverage is earned rather than read from a substring
//
...
struct CoverageBasisTests {
    ...
    func sourcedMentionIsCoverage() {
        // One seat's sourced sentence is a mention. A later change raised the bar again: the room has to
...
struct ResearchClaimTests {
    ...
    func sourcedCoverageCanAnswer() async {
        ...
        #expect(
            (engine.researchSession?.rounds ?? 0) >= 2,
            "the second seat is what turns a mention into the room's coverage")
