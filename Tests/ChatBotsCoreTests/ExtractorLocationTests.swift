// ChatBotsCoreTests — where the document extractors actually live
//
...
// `SystemDocumentExtractor` are defined in `Sources/ChatBotsCore/Attachments/DocumentImport.swift`, which imports
...
@Suite("The document extractors are where the comments say they are")
struct ExtractorLocationTests {
    ...
    func theExtractorsAreWhereTheCommentsSay() throws {
        let importModule = try text(of: "Sources/ChatBotsCore/Attachments/DocumentImport.swift")
        ...
        let attachments = try text(of: "Sources/ChatBotsCore/Attachments/Attachments.swift")
        #expect(attachments.contains("DocumentImport.swift"))
        #expect(attachments.contains("SystemDocumentExtractor"))
        // The CLI is read as a target rather than as one file: the entry point was split into
        // sibling files, and the extractor is named where the attachments are read, not in
        // `main.swift`.
        let cli = sources(under: "Sources/ChatBotsCLI").map(\.1).joined(separator: "\n")
        #expect(cli.contains("SystemDocumentExtractor"), "the CLI no longer names the extractor")
    }

    @Test("The CLI is what installs it, and the app target has no extractor at all")
    func theCliInstallsItAndTheAppDoesNot() throws {
        // Counted across the target, so moving the install into a sibling file still counts it.
        let cli = sources(under: "Sources/ChatBotsCLI").map(\.1).joined(separator: "\n")
        let installs =
            cli.components(
