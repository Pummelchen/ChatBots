// ChatBotsCoreTests — the core's dead declarations are gone or wired (A206)
//
// The Phase B pass found six declarations in the core that were written or named but never used:
...
@Suite("The core's dead declarations are gone or wired (A206)")
struct CoreDeadDeclarationTests {
    ...
    func theDeadDeclarationsAreGone() throws {
        ...
        let engine = try text(of: "Sources/ChatBotsCore/OpenAIResponsesEngine.swift")
    ...
    func theUsefulPartsRemain() throws {
        let conflict = try text(of: "Sources/ChatBotsCore/ConflictState.swift")
        let http = try text(of: "Sources/ChatBotsCore/HTTPServer.swift")
        let engine = try text(of: "Sources/ChatBotsCore/OpenAIResponsesEngine.swift")
        let director = try text(of: "Sources/ChatBotsCore/ResearchDirector.swift")
        let prompt = try text(of: "Sources/ChatBotsCore/PromptBuilder.swift")
