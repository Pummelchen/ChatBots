// ChatBotsCoreTests — the transport listener's session rules.
//
...
@MainActor
@Suite("The transport keeps the reasons its sessions ended")
...
@MainActor
@Suite("A session that never finishes a frame gives its slot back", .serialized, TransportSerialized())
...
@MainActor
@Suite("A frame the server cannot read is answered", .serialized, TransportSerialized())
...
@MainActor
@Suite("A second start does not leave the first listening", .serialized, TransportSerialized())
