...
struct APIEndpointsSheet: View {
    ...
    /// The seat's symbol, name and the switch that puts it on the API backend.
    private func seatHeader(_ seat: Int, pane: AgentPaneState) -> some View {
        HStack(spacing: 8) {
    ...
    /// The URL, model, key and compatibility fields for one seat.
    private func endpointGrid(_ seat: Int) -> some View {
    ...
    /// The resolved responses URL, and the shortcut that copies seat 1's endpoint.
    private func seatFooter(_ seat: Int) -> some View {
        HStack(spacing: 10) {
    ...
    private var webToolsWarning: some View {
        Label(
