    ...
    private var footer: some View {
        HStack(spacing: 10) {
            ...
            Label(contextLabel, systemImage: "text.book.closed")
                .help(
                    "Prompt size against this seat's context window; the log is condensed when it crosses the "
                        + "threshold, so the beginning of the discussion is never silently dropped"
                )
        }
