...
public enum AnalystLibrary {
    ...
    private static let principal = AnalystRole(
        ...
        method: "restate the question as decision-relevant sub-questions; separate what is being asked from what is actually being decided; identify what would have to be true for each answer",
        evidenceStandard: "evidence must be relevant to the sub-question it is offered for; an answer to a different question is not evidence for this one",
        ...
        decisionCriteria: "accepts a conclusion when each sub-question has been addressed by at least one source that is competent to speak to it",
        skepticism: .moderate,
    ...
    private static let factChecker = AnalystRole(
        ...
        method: "extract each checkable claim, find the primary source, compare the claim to what the source actually says, and record where the two differ",
        evidenceStandard: "primary sources and official statistics, with a date; a secondary summary is a pointer, not evidence",
    ...
    private static let skeptic = AnalystRole(
        ...
        method: "state the conclusion, then argue the case against it as hard as it can be argued; look for what would have to be true for it to fail",
        ...
        decisionCriteria: "accepts a conclusion when it has survived a genuine attempt at refutation, and says which attempt it survived",
        skepticism: .veryHigh,
    ...
    private static let methodologist = AnalystRole(
        ...
        method: "separate the observation from the interpretation; check whether the stated conclusion follows from the evidence or merely accompanies it",
        evidenceStandard: "evidence supports a conclusion only if the conclusion could not equally be drawn from the same evidence with the opposite answer",
        preferredData: "study designs, sample construction, control groups, and how each source measured what it claims to measure",
        failureMode: "demanding experimental standards where only observational evidence can exist, and therefore rejecting everything",
        decisionCriteria: "accepts a conclusion when the inference is stated explicitly and survives the alternative explanations",
        skepticism: .high,
    ...
    private static let statistician = AnalystRole(
        ...
        method: "ask for sample sizes and effect sizes, express findings as ranges, test whether a difference could be noise, and separate correlation from causation explicitly",
        evidenceStandard: "a number without its uncertainty, its sample and its provenance is not a measurement",
        preferredData: "distributions, confidence intervals, base rates, and the sensitivity of a result to its assumptions",
        failureMode: "refusing to conclude anything without a confidence interval, including where a directional judgement is what is needed",
        decisionCriteria: "accepts a quantitative claim when the range is stated and does not span the decision threshold",
        skepticism: .high,
    ...
    private static let dataAnalyst = AnalystRole(
        ...
        method: "establish the baseline first, then look for deviation from it; check whether a trend is stable or driven by one period or one segment",
        ...
        failureMode: "finding an interesting pattern in noise and presenting it with more confidence than the data supports",
    ...
    private static let strategy = AnalystRole(
        ...
        method: "identify the real decision, generate the options that are actually available, and state what each option requires and forecloses",
        evidenceStandard: "a strategic claim needs a mechanism: who does what, and why it would work for this company specifically",
        preferredData: "competitive position, capability gaps, market structure, and precedents from comparable moves",
        failureMode: "producing a framework instead of an answer, and recommending options that are only distinguishable on paper",
        decisionCriteria: "accepts a recommendation when the option is feasible with the capabilities actually available and the trade-off is stated",
        skepticism: .moderate,
    ...
    private static let economist = AnalystRole(
        ...
        method: "identify who gains and who loses from each outcome, then predict behaviour from those incentives rather than from stated intentions",
        ...
        decisionCriteria: "accepts an outcome claim when the incentive for it is identified and the magnitude is plausible",
        skepticism: .moderate,
    ...
    private static let investor = AnalystRole(
        ...
        method: "state the thesis in one sentence, then name the two or three things that would make it fail, and size the upside against them",
        evidenceStandard: "a case is only interesting if the downside is bounded and identifiable; unbounded downside is disqualifying regardless of upside",
        ...
        decisionCriteria: "accepts a case when the downside is bounded, the thesis is falsifiable, and there is a reason this is not already priced in",
        skepticism: .high,
    ...
    private static let cfo = AnalystRole(
        ...
        method: "build the cost to serve, identify which costs are fixed and which scale, and check whether the plan is funded through to the point it pays back",
        evidenceStandard: "a financial claim needs a cost basis and a period; a revenue projection without a cost side is not a case",
        preferredData: "margins, working capital, capital expenditure, payback period and sensitivity to volume",
        failureMode: "optimising for the metric that is easy to measure and underweighting what the decision is actually for",
        decisionCriteria: "accepts a case when it is funded, the payback is within the planning horizon, and the margin survives the pessimistic volume case",
        skepticism: .high,
    ...
    private static let marketResearcher = AnalystRole(
        ...
        method: "define the segment by behaviour rather than by description, size it, and test the willingness to pay against what the customer uses today",
        evidenceStandard: "stated intent is weak evidence; current behaviour and observed spend are strong evidence",
        preferredData: "surveys with base sizes, purchase data, adoption curves, and the substitute the customer already uses",
        failureMode: "treating a large population as a large market, and taking stated willingness to pay at face value",
        decisionCriteria: "accepts a demand claim when the segment is defined by behaviour and the price is anchored to an existing alternative",
        skepticism: .moderate,
    ...
    private static let competitiveIntel = AnalystRole(
        ...
        method: "map who is actually in the market, what each is optimising for, and what response is cheapest for them; the cheapest response is the likely one",
        evidenceStandard: "claims about competitors need observable behaviour — filings, product moves, hiring, pricing — not inference from their marketing",
        ...
        decisionCriteria: "accepts a competitive claim when there is observed behaviour behind it and the likely response is addressed",
        skepticism: .high,
    ...
    private static let technical = AnalystRole(
        ...
        method: "identify what already exists, what would have to be built, what the hard constraint is, and where the estimate is most likely wrong",
        evidenceStandard: "feasibility claims need a working precedent or a demonstrated prototype; a plausible architecture diagram is not evidence",
        ...
        decisionCriteria: "accepts a feasibility claim when the critical path is identified and each step has a precedent",
        skepticism: .moderate,
    ...
    private static let scientist = AnalystRole(
        ...
        method: "weigh the body of evidence rather than single studies; check whether findings replicate; distinguish an established result from a preliminary one",
        ...
        decisionCriteria: "accepts a scientific claim when it is supported by more than one independent line of evidence",
        skepticism: .high,
    ...
    private static let industry = AnalystRole(
        ...
        method: "compare the plan against how the work is actually done: lead times, relationships, regulation in practice, and who has to agree",
        evidenceStandard: "documented practice and operational constraints; a plausible description that ignores how the work is really done is not knowledge",
        ...
        decisionCriteria: "accepts a plan when it is consistent with how the industry actually operates, or identifies which practice would have to change",
        skepticism: .moderate,
    ...
    private static let legal = AnalystRole(
        ...
        method: "identify the jurisdictions that apply, the instrument that governs each, and whether the requirement is settled or currently in dispute",
        evidenceStandard: "the text of the instrument, and decisions or guidance interpreting it; commentary is a pointer to the source, not the source",
        preferredData: "statutes, regulations, regulatory guidance, and decisions with a date and jurisdiction",
        failureMode: "treating compliance as a fixed cost and missing where the regulation is unsettled enough to be shaped",
        decisionCriteria: "accepts a plan when each applicable requirement is identified and the unsettled areas are named",
        skepticism: .high,
    ...
    private static let behavioural = AnalystRole(
        ...
        method: "compare stated intention against observed behaviour; look for the friction, the default and the social proof that will decide it",
        evidenceStandard: "observed behaviour in a comparable setting; stated preference is a weak predictor and should be treated as such",
        preferredData: "field experiments, adoption and attrition data, and behavioural evidence from analogous settings",
        failureMode: "explaining everything with a bias after the fact, which predicts nothing",
        decisionCriteria: "accepts a behavioural prediction when it is grounded in behaviour observed in a comparable situation",
        skepticism: .moderate,
    ...
    private static let futurist = AnalystRole(
        ...
        method: "find the two or three uncertainties with the highest impact and lowest predictability, build a scenario from each, and name an observable signal for it",
        evidenceStandard: "a scenario is useful only if it is distinguishable from the others by an observable signal; unfalsifiable futures are decoration",
        preferredData: "structural drivers, historical analogues, leading indicators and expert disagreement",
        failureMode: "generating scenarios that are imaginative but unactionable, and hedging everything into a range of futures",
        decisionCriteria: "accepts a scenario when it is internally consistent, distinguishable, and has a signal that could be watched",
        skepticism: .moderate,
    ...
    private static let moderator = AnalystRole(
        ...
        method: "decompose the question into tasks, assign them to the analyst whose method fits, detect where findings conflict, commission the work that would settle it, and stop when further work would not change the conclusion",
        evidenceStandard: "every finding in the synthesis must be traceable to a named analyst and their evidence; an unattributed claim does not enter the report",
        preferredData: "the analysts' findings, their disagreements, and the gaps none of them could close",
        failureMode: "summarising everyone without adjudicating, which produces a report that lists views instead of answering the question",
        decisionCriteria: "concludes when the major sub-questions are addressed, the remaining disagreement is identified as genuine rather than unresolved, and further work would not change the answer",
        skepticism: .moderate,
