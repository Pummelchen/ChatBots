...
//
// Extracted from `DocumentImport` when a second unit needed it: it was written for the document
...
public enum SystemProcess {
    ...
    public static func run(
        ...
        if collected.outputExceeded {
            ...
            // likely still writing, and one zombie per oversized conversion is the leak this guard closes.
            Self.stop(process)
    ...
    /// same read is dropped so a full pipe cannot block the child either.
