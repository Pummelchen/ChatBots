...
public struct StreamPacer: Sendable {
    ...
    /// whole mechanism exists to remove. `StreamPacerPool` remembers the last
...
public final class StreamPacerPool {
    ...
    /// to the turn, so it is what seeds the next turn's pacer. Written by
    ...
    public func enqueue(_ text: String, agentID: String, channel: Channel) {
        ...
        // a slow stream drained its queue at the start of every turn.
