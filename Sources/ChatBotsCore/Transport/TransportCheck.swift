...
public struct TransportCheckReport: Sendable {
    ...
    // installer's smoke test that can be tested at all. `run` still owns the
...
public enum TransportCheck {
    ...
    /// installer's smoke test runs had no deadline of its own.
    ...
    /// the engine needs to come up instead of the first one spending it all.
    ...
    public static func run(
        ...
        engineProcess.arguments = [
            ...
            // caller's directory rather than about the transport.
