...
public actor OpenAIResponsesEngine: LLMEngine {
    ...
    /// long as the engine does, which is what makes the connection pool reusable.
    ...
    /// `load` then reported as "the server has no model loaded — load one in LM Studio": a server
    /// that answered was described as a server with no model, and the moderator
    ...
    public func generate(
        ...
        // this does too, and statistics are not recorded for a turn that failed.
