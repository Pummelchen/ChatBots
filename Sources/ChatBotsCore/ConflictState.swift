...
public struct Relationship: Sendable, Hashable, Codable {
    ...
    mutating func clamp() {
        ...
        if var grudge { grudge.intensity = min(1, max(0, grudge.intensity)); self.grudge = grudge }
        if var alliance { alliance.strength = min(1, max(0, alliance.strength)); self.alliance = alliance }
    }
}
