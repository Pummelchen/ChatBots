...
public enum WebAssets {
    ...
    public static func asset(for path: String) -> Asset? {
        ...
        case "/style-panes.css":
            return Asset(contentType: "text/css; charset=utf-8", body: Data(stylePanesCSS.utf8))
        case "/style-panels.css":
            return Asset(contentType: "text/css; charset=utf-8", body: Data(stylePanelsCSS.utf8))
        case "/deltas.js":
            return Asset(contentType: "application/javascript; charset=utf-8", body: Data(deltasJS.utf8))
        case "/votes.js":
            return Asset(contentType: "application/javascript; charset=utf-8", body: Data(votesJS.utf8))
        case "/app.js":
            return Asset(contentType: "application/javascript; charset=utf-8", body: Data(appJS.utf8))
        case "/app-core.js":
            return Asset(contentType: "application/javascript; charset=utf-8", body: Data(appCoreJS.utf8))
        case "/app-screen.js":
            return Asset(contentType: "application/javascript; charset=utf-8", body: Data(appScreenJS.utf8))
        case "/app-transcript.js":
            return Asset(contentType: "application/javascript; charset=utf-8", body: Data(appTranscriptJS.utf8))
        case "/app-controls.js":
            return Asset(contentType: "application/javascript; charset=utf-8", body: Data(appControlsJS.utf8))
        case "/app-lineup.js":
            return Asset(contentType: "application/javascript; charset=utf-8", body: Data(appLineupJS.utf8))
        case "/app-commands.js":
            return Asset(contentType: "application/javascript; charset=utf-8", body: Data(appCommandsJS.utf8))
        case "/favicon.ico":
            return nil
        default:
            return nil
...
<!-- The same cascade, split by concern. Order matters and is the order the single stylesheet had. -->
<link rel="stylesheet" href="/style-panes.css">
...
<!-- The attributes the client fills in. `data-device` is derived from the live viewport and
     drives every responsive rule; `data-layout` is the user's mode choice. Both are set by the
     client before first paint where possible, and CSS has sane defaults if it is not. -->
...
<!-- `deltas.js` and `votes.js` stay classic scripts because Node `require()`s them in
     `tools/check-web-deltas.js` and `tools/check-web-votes.js`; `app.js` is an ES module and
     imports the rest of the page. A module is deferred, so it runs after both, which is the
     order these three have always had. -->
 ...
 *
 * This is the base layer: the variables, the reset and the controls. `style-panes.css` carries
 * the panes and the messages and `style-panels.css` the overlays; index.html links the three in
 * that order, so the cascade is the one this file had when it was all in one place.
 */
...
"""
    ...
    static let stylePanesCSS = """
/* ChatBots — panes and the transcript.
 *
 * The second of the three stylesheets. Load order is the order index.html links them, so the
 * cascade is unchanged from when this was one file: base and controls, then the panes and the
 * messages in them, then the panels and the forced phone view.
 */
...
"""
    ...
    static let stylePanelsCSS = """
/* ChatBots — the panels: research, kept conversations, the line-up and the persona picker.
 *
 * The last of the three stylesheets. It follows `style.css` and `style-panes.css`, which is
 * what keeps the cascade identical to the single file these were split from.
 */
