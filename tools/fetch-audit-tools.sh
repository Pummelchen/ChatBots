...
# three need digests.
#
# usage: tools/fetch-audit-tools.sh --into <dir>     fetch, verify and unpack into <dir>
#        tools/fetch-audit-tools.sh --verify-only    fetch and verify only, unpacking nothing
...
# checked byte-for-byte against the downloaded files (both figures are in
# `AUDIT/baseline/swift64/a187-audit-tools.log`). To move a version, change it here and re-derive all
